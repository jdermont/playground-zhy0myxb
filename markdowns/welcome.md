...
N-Tuple Neural Network is a set of patterns and each pattern contains some value. The network prediction is just summing those values and squashing with activation function. It acts much like perceptron and can be trained as such, mostly using (stochastic) gradient descent.
...
In this article I will show how this network can be trained for tic-tac-toe, so that 1-ply search bot will become a perfect player, despite only searching 1 ply ahead instead of brute-forcing all possible states until the end of game.

# N-Tuple Network for tic tac toe

There are various ways to generate patterns for games. They could be hand-crafted or automatically generate by random snake-like walk. Each game will benefit from different shapes of patterns. The goal of tic tac toe is to get 3 in row of own pieces - vertically, horizontally or diagonally.  I think the N-Tuple Network for such game should be naturally the all eight possible rows of 3. 3 horizontal rows, 3 vertical rows and the 2 diagonals.

(Image/Code)

Each state of row can be encoded in lookup table (LUT in some literature) as index. In this case we have base3 system, empty being 0, X being 1 and O being 2.

(Image/Code)

This way each row has 27 (3\*\*3) possible states. But for many games, to know which side is to move now, is very crucial. In tic tac toe consider there is a row XX_. For player X it is quite advantageous. But if we knew the side to move now we could predict more accurately how good it is for X. If the current player is X, this is clearly a win. If the current player is O, O must play in this row, unless O has its own row like _OO. So let's add side to move information to the network. Either we can use 2 separate N-Tuple Networks for side to move, or double the possible states for each row, which is effectively the same. I chose the latter. That is, if side to move is player O, I add 27 to the indexes.
...
**Note:** This network is from perspective of player X (X wins = 1, O wins = -1). When used in minimax bot, the player O would take the minus of the predicted output.

# Training with data

There are several ways to train this network. This could be one of reinforcement learning like temporal difference learning, or supervised learning based on labeled examples. Tic tac toe is a solved game. Moreover, it is strongly solved game and we can know true value (win, loss, draw) for each possible position in game. So the training will be supervised learning based on true values of positions.
...
Generally speaking, we feed training data, the positions and their values, into the network so it can adjust its weights to minimize the error in the prediction. In training data, the positions with XXX rows will have value of 1, so likely the network will adjust weights so that XXX rows will be relatively big. 
...
The trained N-Tuple Network, which we believe is pretty good at evaluating game positions, can be used as evaluation function for the minimax bot. Normally perfect tic tac toe minimax bots use brute force search until the end of game without evaluation except for win/lose/draw situations. Tic tac toe isn't big game so it's possible. My experiments show that with this network, trained only for few seconds, and search with only 1 ply, it is sufficient act like perfect bot. 
